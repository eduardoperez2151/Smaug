...
*/
public class CompareGreaterOrEqual extends Exp {
	...
	@Override public boolean equals(Object obj) {
		...
		CompareGreaterOrEqual other = (CompareGreaterOrEqual)obj;
		return (this.left == null ? other.left == null : this.left.equals(other.left))
			&& (this.right == null ? other.right == null : this.right.equals(other.right));
	}

	public static CompareGreaterOrEqual generate(Random random, int min, int max) {
		Exp left; Exp right; 
		left = Exp.generate(random, min-1, max-1);
		right = Exp.generate(random, min-1, max-1);
		return new CompareGreaterOrEqual(left, right);
	}
}
